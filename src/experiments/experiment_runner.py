...
class ExperimentRunner:
    ...
    def load_dataset(self, csv_path: str = 'data/processed/evaluation.csv'):
        ...
        try:
            ...
            for label in ['positive', 'negative', 'neutral']:
                ...
                print(f"  {label:10s} {bar} {pct:5.1f}%")
            print()
            
            # Convert to list of dicts
            self.eval_data = df.to_dict('records')
            
            print(f"Total evaluation samples: {len(self.eval_data)}\n")
    ...
    def initialize_evaluation(self):
        ...
        print(f"  Fitness metric: {self.config.fitness_metrics}\n")
    ...
    def initialize_optimizer(self):
        ...
        print(f"  Gc: {self.config.Gc}, Pdp: {self.config.Pdp}\n")
    ...
    def create_fitness_function(self):
        ...
        def fitness_fn(squirrel):
            ...
            try:
                ...
                print(f"[WARNING] Error evaluating squirrel: {e}")
    ...
    def run_optimization(self):
        ...
        print(f"Dataset: {len(self.eval_data)} evaluation samples")
        print(f"Population: {self.config.population_size} squirrels")
        print(f"Max iterations: {self.config.max_iterations}")
        print(f"LLM: {self.config.ollama_model}\n")
        ...
        print(f"\nOptimization complete! Improvements: {history['improvements']}")
        print(f"Best fitness: {best_squirrel.fitness:.6f}\n")
    ...
    def evaluate_best_prompt(self, best_squirrel):
        ...
        print(f"  Fitness (error): {eval_fitness:.6f}")
        print(f"  Accuracy:        {1 - eval_fitness:.4f}\n")
    ...
    def analyze_results(self):
        ...
        print("="*60)
        print("PROOF 1: FITNESS IMPROVEMENT")
        print("="*60)
        ...
        print(f"  Number of improvements: {history['improvements']}")
        
        if improvement > 0:
            print("  ✓ SSA IMPROVED fitness over iterations")
        else:
        ...
        print("="*60)
        print("PROOF 2: MONOTONIC CONVERGENCE")
        print("="*60)
        print(f"  Best fitness never increased: {is_monotonic}")
        if is_monotonic:
            print("  ✓ Algorithm correctly tracks best solution")
        print()
        ...
        print("="*60)
        print("PROOF 3: POPULATION CONVERGENCE")
        print("="*60)
        print(f"  Initial population std: {initial_std:.6f}")
        print(f"  Final population std:   {final_std:.6f}")
        print(f"  Diversity reduction:    {diversity_reduction:.2f}%")
        
        if diversity_reduction > 0:
            print("  ✓ Population converged (squirrels found good region)")
        else:
        ...
        if total_improvement > 0:
            ...
            if milestones['50%'] is not None:
                ...
                if early_pct < 30:
                    print("  ✓ Fast initial convergence (exploration → exploitation)")
        print()
        ...
        if final_gap < initial_gap:
            print("  ✓ Population following best solution (exploitation working)")
        print()
        ...
        if improvement > 0:
            evidence_count += 1
            print("  ✓ Fitness improved over iterations")
        else:
        ...
        if is_monotonic:
            evidence_count += 1
            print("  ✓ Best solution properly tracked")
        else:
            print("  ✗ Best tracking issue")
        ...
        if history['improvements'] >= 3:
            evidence_count += 1
            print(f"  ✓ Multiple improvements ({history['improvements']})")
        else:
        ...
        if accuracy_gain >= 5:
            evidence_count += 1
            print(f"  ✓ Meaningful accuracy gain ({accuracy_gain:.1f}%)")
        else:
    ...
    def run_full_experiment(self):
        ...
        try:
            ...
            print("="*80)
            print(f"\nResults saved to: {self.dirs['results']}")
            print(f"Figures saved to: {self.dirs['figures']}\n")
